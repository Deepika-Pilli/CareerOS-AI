...
function parseCurrentSkills(input) {
  return input
    .split(/[,;\n]+/)
    .map(normalizeSkill)
    .filter((s) => s.length > 0);
...
function skillMatches(userSkill, requirement) {
...
function findMatch(requirement, userSkills) {
...
function performSkillGapAnalysis(targetRole, currentSkillsInput) {
  ...
  const estimatedLearningDuration = formatDuration(Math.round(totalWeeks * 0.7));
...
export const analyzeSkillGap = async (req, res) => {
  try {
    ...
    const analysis = performSkillGapAnalysis(targetRole, currentSkills);

    const skillGap = await SkillGap.create({
      userId,
      targetRole: analysis.targetRole,
      matchPercent: analysis.matchPercent,
      skillsAvailable: analysis.skillsAvailable,
      missingSkills: analysis.missingSkills,
      learningPriority: analysis.learningPriority,
      recommendedResources: analysis.recommendedResources,
      estimatedLearningDuration: analysis.estimatedLearningDuration,
    });
    ...
    return res.status(500).json({
      success: false,
      message: "Server error during skill gap analysis",
    });
  }
};
