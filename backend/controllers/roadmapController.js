...
const CURRENT_STATUS_OPTIONS = ["Student", "Fresher"];
...
function formatTimeline(months, status) {
...
function performRoadmapGeneration(currentStatus, targetRole) {
...
export const generateRoadmap = async (req, res) => {
  try {
    const { targetRole, currentStatus } = req.body;

    if (!targetRole || !currentStatus) {
      return res.status(400).json({
        success: false,
        message: "Please provide targetRole and currentStatus",
      });
    }
    ...
    if (!CURRENT_STATUS_OPTIONS.includes(currentStatus)) {
      return res.status(400).json({
        success: false,
    ...
    const roadmap = performRoadmapGeneration(currentStatus, targetRole);
    ...
    const savedRoadmap = await Roadmap.create({
      ...
      interviewPrepPlan: roadmap.interviewPrepPlan,
