...
const QUESTION_BANK: Record<TargetRole, QuestionTemplate[]> = {
  "Frontend Developer": [
...
const EMPTY_MESSAGE = "Please answer this question.";
...
function getTargetWordCount(difficulty: Difficulty): number {
  if (difficulty === "Easy") return 50;
  if (difficulty === "Medium") return 75;
  return 100;
}

function scoreAnswerLength(answer: string, difficulty: Difficulty): number {
...
function scoreTechnicalKeywords(answer: string, role: TargetRole, question: InterviewQuestion): number {
...
function scoreSentenceStructure(answer: string): number {
...
function scoreConfidenceFromMetrics(metrics: AnswerMetrics, answer: string): number {
...
function scoreCommunicationFromMetrics(metrics: AnswerMetrics): number {
  return clamp(metrics.structure * 0.55 + metrics.length * 0.3 + metrics.keywords * 0.15);
}

function scoreTechnicalFromMetrics(metrics: AnswerMetrics, question: InterviewQuestion): number {
  const typeWeight =
...
function buildQuestionSuggestions(
  ...
  scores: { confidence: number; communication: number; technical: number },
): string[] {
...
export function generateQuestions(role: TargetRole, difficulty: Difficulty): InterviewQuestion[] {
  ...
  const types: QuestionType[] = ["behavioral", "technical", "situational"];
...
function questionCompositeScore(e: QuestionEvaluation): number {
  return (e.confidenceScore + e.communicationScore + e.technicalScore) / 3;
}
...
export function evaluateInterview(
  ...
  const avg = (key: keyof Pick<QuestionEvaluation, "confidenceScore" | "communicationScore" | "technicalScore">) =>
    evaluations.length
  ...
  const overallInterviewScore =
    evaluations.length > 0
      ? Math.round(
          evaluations.reduce((sum, e) => sum + questionCompositeScore(e), 0) / evaluations.length,
        )
      : 0;
...
export function generateQuestionsWithDelay(
  role: TargetRole,
  difficulty: Difficulty,
  ms = 1000,
): Promise<InterviewQuestion[]> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(generateQuestions(role, difficulty)), ms);
  });
}

export function evaluateInterviewWithDelay(
  role: TargetRole,
  difficulty: Difficulty,
  questions: InterviewQuestion[],
  answers: Record<string, string>,
  ...
  return new Promise((resolve) => {
    setTimeout(() => resolve(evaluateInterview(role, difficulty, questions, answers)), ms);
  });
}
